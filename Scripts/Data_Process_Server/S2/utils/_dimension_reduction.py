...
def reduce_dimension(inputdata, index_col, file_name="dissum_Autoencoder", new_dimension=128):
    ...
    model.add(Dense(num_dimension,  activation='sigmoid'))
    model.summary()
...
def get_mimic_five_repre128():
    ...
    all_data=left_join(all_data,epis_demo_df,"HADM_ID").drop('SUBJECT_ID', 1).fillna(0).set_index("HADM_ID")
    # print(all_data.columns)

    reduce_dimension(
        all_data.loc[:, (all_data!= 0).any(axis=0)],
